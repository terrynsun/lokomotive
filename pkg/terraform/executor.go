...
// Execute executes the provided ExecutionSteps sequentially in the order they appear in the slice.
// If a step has a PreExecutionHook defined, the hook is run prior to executing the step. If any
// error is encountered, the error is returned and the entire execution is halted.
...
func (ex *Executor) execute(verbose bool, args ...string) error {
	...
	if err != nil {
		return fmt.Errorf("executing Terraform command with arguments '%s' in directory %s: %w", strings.Join(args, " "), ex.WorkingDirectory(), err)
	}
